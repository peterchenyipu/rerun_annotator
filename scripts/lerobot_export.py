...
sample_5_path = Path(
    os.environ.get("RERUN_SAMPLE_PATH", repo_root / "tests" / "assets" / "rrd" / "sample_5")
)
output_root = Path(os.environ.get("LEROBOT_OUTPUT_ROOT", repo_root / "outputs" / "lerobot"))

single_repo_id = "rerun/droid_lerobot"
full_repo_id = "rerun/droid_lerobot_full"
single_output_dir = output_root / single_repo_id
full_output_dir = output_root / full_repo_id
...
if not sample_5_path.exists():
    raise FileNotFoundError(
        f"Sample dataset path does not exist: {sample_5_path}. "
        "Set RERUN_SAMPLE_PATH to a directory of RRD recordings before running this script."
    )
...
reset_output_dir(single_output_dir)
lerobot_dataset = LeRobotDataset.create(
    repo_id=single_repo_id,  # Dataset identifier
    fps=config.fps,
    features=features,
    root=single_output_dir,
    use_videos=config.use_videos,
)
print(f"Saving single-episode dataset to: {single_output_dir}")
...
reset_output_dir(full_output_dir)
lerobot_dataset = LeRobotDataset.create(
    repo_id=full_repo_id,
    fps=config.fps,
    features=features,
    root=full_output_dir,
    use_videos=config.use_videos,
)
print(f"Saving multi-episode dataset to: {full_output_dir}")
