...
def _save_episode_without_video_decode(
    ...
    *,
    ...
    writer_api = writer if writer is not None else lerobot_dataset

    episode_buffer = getattr(writer_api, "episode_buffer", None)
    if episode_buffer is None:
        create_episode_buffer = getattr(writer_api, "_create_episode_buffer", None)
        if create_episode_buffer is None:
            create_episode_buffer = getattr(writer_api, "create_episode_buffer")
        episode_buffer = create_episode_buffer()
        setattr(writer_api, "episode_buffer", episode_buffer)
    ...
    episode_metadata = writer_api._save_episode_data(episode_buffer)
    ...
    for spec in specs:
        ...
        with suppress_ffmpeg_output():
            ...
            video_metadata.update(
                writer_api._save_episode_video(
                    video_key,
                    episode_index,
                    temp_path=temp_path,
                )
    ...
    if create_episode_buffer is None:
        create_episode_buffer = getattr(writer_api, "create_episode_buffer")
    setattr(writer_api, "episode_buffer", create_episode_buffer())
